...
#   - Appendix Figure S14 uses rank-2 zones from code 8 ranked_zone.tif files.
...
load_zone_lookup <- function() {
  ...
  pal_file <- pick_file(pal_files, prefer = c("color_palette_China.csv", "palette"))
  if (!is.na(pal_file)) {
  ...
  lookup[zoneID == novel_value, `:=`(zone_label = "Novel ecotype", color = "#333333")]
  lookup[]
}
...
plot_zone_panel_gg <- function(
  ...
  if (categorical) {
    ...
    p <- ggplot(plot_dt, aes(x = x, y = y, fill = value_chr)) +
      ...
      labs(title = plot_title, fill = "Zone") +
    ...
    p <- ggplot(plot_dt, aes(x = x, y = y, fill = value)) +
      ...
      labs(title = plot_title, fill = "Suitability") +
      theme_void(base_size = 10) +
      theme(
        plot.title = element_text(face = "bold", hjust = 0, size = 13),
        strip.text = element_text(face = "bold", size = 9.5),
...
find_binary_summary_file <- function(niche_type) {
  ...
  files <- files[!grepl("(normal_map|confusion|assigned|future tree niche|population|species|ranking|visualization|area|lookup|index|multiclass|multi_class|robust)", files, ignore.case = TRUE)]
...
read_one_binary_rf_summary <- function(niche_type, file) {
  ...
  out <- out[workflow %in% binary_workflow_order & !is.na(value) & is.finite(value)]
  out[]
}
...
run_step("Appendix Table S1 | Result file inventory", {
  files <- unique(c(find_files("\\.csv$"), find_files("\\.tif$")))
      ...
      fifelse(
        ...
        fifelse(
          ...
          fifelse(
            ...
            fifelse(grepl("confusion|normal_map", file, ignore.case = TRUE), "reference assessment", "other")
          )
        )
      )
    )
...
run_step("Appendix Figure S1 | Binary RF zone-level metric heatmaps", {
  ...
  if (nrow(dt) == 0) stop("No selected binary RF metrics available.")
  ...
  p <- ggplot(dt, aes(x = zoneID_chr, y = workflow_label, fill = value)) +
    ...
    labs(title = "Zone-level binary RF performance", x = "Vegetation zone", y = NULL) +
  ...
  save_gg(p, file.path(fig_dir, "Appendix_FigureS1_binary_RF_zone_metric_heatmaps.png"), width = 15, height = 6.5)
})
...
run_step("Appendix Figure S2 | Climate vs soil zone-level metric scatter", {
  ...
  if (file.exists(metric_file)) dt <- fread(metric_file) else dt <- make_binary_rf_metrics()
  metrics_keep <- c("AUC", "F1", "Accuracy", "Balanced accuracy")
  dt <- dt[metric %in% metrics_keep]
  wide <- dcast(dt, workflow + zoneID + metric ~ niche_type, value.var = "value", fun.aggregate = mean)
  wide <- wide[!is.na(climate) & !is.na(soil) & is.finite(climate) & is.finite(soil)]
  if (nrow(wide) == 0) stop("No matched climate/soil metric pairs found.")
  ...
  p <- ggplot(wide, aes(x = climate, y = soil, colour = zoneID_chr)) +
    ...
    theme_app(base_size = 9.5)
  save_gg(p, file.path(fig_dir, "Appendix_FigureS2_climate_vs_soil_metric_scatter.png"), width = 12, height = 8.5)
})
...
run_step("Appendix Figure S3 | Reference maps for all workflows", {
  ...
  if (length(files) <= 1) stop("No predicted reference maps found.")
  plot_zone_panel_gg(
    files = files,
    titles = titles,
...
run_step("Appendix Figure S4 | Reference-map confusion matrix heatmap", {
  ...
  if (is.na(from_col) || is.na(to_col) || is.na(count_col)) stop("Cannot identify confusion columns.")
  cm <- data.table(
    original_zone = extract_zone_id(dt[[from_col]]),
    predicted_zone = extract_zone_id(dt[[to_col]]),
    count = as.numeric(dt[[count_col]])
  )
  cm <- cm[original_zone %in% model_zoneID & predicted_zone %in% c(model_zoneID, novel_value) & !is.na(count) & is.finite(count)]
  cm <- cm[, .(count = sum(count, na.rm = TRUE)), by = .(original_zone, predicted_zone)]
  ...
  p <- ggplot(cm, aes(x = predicted_zone, y = original_zone, fill = prop)) +
    geom_tile(colour = "white", linewidth = 0.15) +
    scale_fill_gradientn(colours = grDevices::hcl.colors(60, palette = "YlGnBu"), name = "Row proportion") +
    labs(title = paste0("Reference-map confusion matrix: ", preferred_method), x = "Predicted zone", y = "Original zone") +
    theme_app(base_size = 9.5) +
    theme(
  ...
  save_gg(p, file.path(fig_dir, "Appendix_FigureS4_reference_confusion_matrix_heatmap.png"), width = 9.5, height = 8.5)
})
...
run_step("Appendix Figure S5 | Zone metrics for all binary workflows", {
  ...
  if (length(files) == 0) stop("No normal_map_zone_metrics.csv found.")
  all_dt <- list()
  for (f in files) {
    dt <- tryCatch(fread(f), error = function(e) NULL)
    if (is.null(dt) || nrow(dt) == 0) next
    method_col <- pick_col(dt, c("method", "model", "workflow", "model_version", "version"))
    if (!is.na(method_col)) dt[, method := as.character(get(method_col))] else dt[, method := infer_workflow(f)]
    ...
    mm <- melt(dt, id.vars = c("method", "zoneID"), measure.vars = metric_cols, variable.name = "metric", value.name = "value")
    mm[, metric := fifelse(norm_name(metric) == "f1_score", "F1", tools::toTitleCase(metric))]
    all_dt[[length(all_dt) + 1L]] <- mm
  }
  if (length(all_dt) == 0) stop("No zone-level metric rows found.")
  ...
  p <- ggplot(m, aes(x = zoneID_fac, y = value, colour = zoneID_chr)) +
  ...
  save_gg(p, file.path(fig_dir, "Appendix_FigureS5_zone_metrics_all_workflows.png"), width = 13, height = 7.5)
})
...
run_step("Appendix Figure S6 | Future ecotype maps by workflow", {
  for (m in map_method_order) {
    future_files <- sapply(future_order, function(s) find_assigned_map(m, s), USE.NAMES = TRUE)
    ...
    plot_zone_panel_gg(
      ...
      outfile = file.path(fig_dir, paste0("Appendix_FigureS6_future_ecotype_maps_", m, ".png")),
      categorical = TRUE,
      population_mode = FALSE,
      show_legend = FALSE,
      ncol = 3,
      width = 11.5,
      height = 7.2,
      plot_title = paste0("Future ecotype maps: ", map_method_labels[m])
    )
  }
})
...
run_step("Appendix Figure S7 | Novel area trajectory across workflows", {
  ...
  if (length(area_list) == 0) stop("No assigned maps available for novel-area calculation.")
  ...
  cat0("[SAVED] ", out_file)
  novel_dt <- area_dt[zoneID == novel_value, .(area_km2 = sum(area_km2, na.rm = TRUE)), by = .(method, method_label, scenario, scenario_label, scenario_order)]
  all_combo <- unique(area_dt[, .(method, method_label, scenario, scenario_label, scenario_order)])
  novel_dt <- merge(all_combo, novel_dt, by = c("method", "method_label", "scenario", "scenario_label", "scenario_order"), all.x = TRUE)
  ...
  p <- ggplot(novel_dt, aes(x = scenario_label, y = area_million_km2, group = method_label, colour = method_label)) +
  ...
  save_gg(p, file.path(fig_dir, "Appendix_FigureS7_novel_area_trajectory_all_workflows.png"), width = 9, height = 5.2)
})
...
run_step("Appendix Figure S8 | All-zone area trajectories for optimized workflow", {
  ...
  if (file.exists(area_file)) area_dt <- fread(area_file) else stop("Area table not found. Run Appendix Figure S7 step first.")
  dt <- area_dt[method == preferred_method]
  if (nrow(dt) == 0) stop("No area rows for preferred method: ", preferred_method)
  ...
  p <- ggplot(dt, aes(x = scenario_label, y = area_million_km2, group = zoneID_chr, colour = zoneID_chr)) +
    ...
    theme_app() +
    theme(axis.text.x = element_text(angle = 35, hjust = 1))
  save_gg(p, file.path(fig_dir, "Appendix_FigureS8_all_zone_area_trajectories_optimized_rf.png"), width = 9.5, height = 5.5)
})

# 15. Appendix Figure S9 ========================================================

run_step("Appendix Figure S9 | Dual-suitability ranking summary maps", {
  if (!dir.exists(ranking_root)) stop("Ranking folder not found: ", ranking_root)
  ...
  for (s in scenarios) {
    ...
    if (any(cat_flags)) {
      plot_zone_panel_gg(
        ...
        outfile = file.path(fig_dir, paste0("Appendix_FigureS9_rank1_zone_", safe_name(s), ".png")),
        categorical = TRUE,
        population_mode = FALSE,
        show_legend = TRUE,
        ncol = 1,
    ...
    if (any(!cat_flags)) {
      plot_zone_panel_gg(
        files = files[!cat_flags],
        titles = titles[!cat_flags],
        outfile = file.path(fig_dir, paste0("Appendix_FigureS9_ranking_summary_", safe_name(s), ".png")),
        categorical = FALSE,
        population_mode = FALSE,
        show_legend = TRUE,
        ncol = 2,
...
run_step("Appendix Table S2 and Figure S10 | Full species population diversity", {
  ...
  if (is.na(f)) stop("No species_zone_population_long.csv found.")
  dt <- fread(f)
  ...
  if (is.na(species_col) || is.na(zone_col)) stop("Cannot identify species and zone columns.")
  ...
  if (!is.na(abundance_col) && is.numeric(dt[[abundance_col]])) {
    ...
    pop <- unique(data.table(species = as.character(dt[[species_col]]), zoneID = extract_zone_id(dt[[zone_col]])))
    pop[, abundance := 1]
  }
  pop <- pop[!is.na(species) & zoneID %in% model_zoneID & !is.na(abundance) & is.finite(abundance)]
  if (nrow(pop) == 0) stop("No valid species-population records.")
  pop_agg <- pop[, .(abundance = sum(abundance, na.rm = TRUE)), by = .(species, zoneID)]
  ...
  p <- ggplot(pdt, aes(x = species, y = n_populations)) +
    ...
    theme_app()
  save_gg(p, file.path(fig_dir, "Appendix_FigureS10_species_population_diversity.png"), width = 8, height = 7)
})
...
run_step("Appendix Figure S11 | Full species x zone abundance heatmap", {
  ...
  pop <- pop[!is.na(species) & zoneID %in% model_zoneID & !is.na(abundance) & is.finite(abundance)]
  pop <- pop[, .(abundance = sum(abundance, na.rm = TRUE)), by = .(species, zoneID)]
  species_order <- pop[, .(n_pop = .N, total = sum(abundance)), by = species][order(-n_pop, -total)]$species
  ...
  p <- ggplot(pop, aes(x = zoneID_chr, y = species, fill = log_abundance)) +
    geom_tile(colour = "white", linewidth = 0.12) +
    scale_fill_gradient(low = "#F5F5F2", high = "#3F4A4A", name = "log(1 + abundance)") +
    labs(title = "Full species-by-ecotype population abundance matrix", x = "Vegetation zone", y = NULL) +
  ...
  save_gg(p, file.path(fig_dir, "Appendix_FigureS11_full_species_zone_abundance_heatmap.png"), width = 11, height = 12)
})

# 18. Appendix Figure S12 =======================================================

run_step("Appendix Figure S12 | Additional population niche maps", {
  roots <- c(file.path(base_dir, "future tree niche"), file.path(base_dir, "future tree niche dual suitability"))
  roots <- roots[dir.exists(roots)]
  if (length(roots) == 0) stop("No future tree niche folders found.")
  files <- unique(unlist(lapply(
    roots,
    function(root) {
      list.files(root, pattern = "\\.tif$", recursive = TRUE, full.names = TRUE, ignore.case = TRUE)
    }
  )))
  files <- files[
  ...
  files <- files[file.exists(files)]
  if (length(files) == 0) stop("No population niche raster files found.")
  score <- rep(0L, length(files))
...
run_step("Appendix Figure S13 | Additional species-level niche maps", {
  ...
  files <- files[file.exists(files)]
  if (length(files) == 0) stop("No species-level niche raster files found.")
  score <- rep(0L, length(files))
  score <- score + as.integer(grepl(preferred_method, files, ignore.case = TRUE, fixed = TRUE))
  score <- score + as.integer(grepl("SSP585", files, ignore.case = TRUE, fixed = TRUE))
  score <- score + as.integer(grepl("2071-2100", files, ignore.case = TRUE, fixed = TRUE))
...
# 20. Appendix Figures S14-S17 ================================================

run_step("Appendix Figures S14-S17 | Ranked ecosystem niche maps by workflow", {
  ...
  find_ranked_zone_file <- function(method, scenario) {
    f0 <- file.path(ranking_root, method, scenario, "ranked_zone.tif")
    if (file.exists(f0)) return(f0)
    
    files <- find_files(
      "ranked_zone.*\\.tif$",
      root = file.path(ranking_root, method, scenario)
    )
    
    pick_file(
      files,
      prefer = c("ranked_zone.tif", method, scenario)
    )
  }
  
  get_ranked_zone_layer <- function(method, scenario, rank_k = 2L) {
    f <- find_ranked_zone_file(method, scenario)
    ...
    rz <- rast(f)
    rank_name <- paste0("rank", rank_k, "_zone")
    
    if (rank_name %in% names(rz)) {
      out <- rz[[rank_name]]
    } else {
      if (nlyr(rz) < rank_k) {
        return(NULL)
  ...
  required_scenarios <- c("normal", ssp245_order, ssp585_order)
  index_list <- list()
  
  for (m in map_method_order) {
    
    cat0("  Checking ranked ecosystem niche maps: ", m)
    
    file_check <- data.table(
      method = m,
      method_label = map_method_labels[m],
      scenario = required_scenarios,
      ranked_zone_file = sapply(
        required_scenarios,
        function(s) find_ranked_zone_file(m, s)
      )
    )
    
    file_check[, exists := !is.na(ranked_zone_file) & file.exists(ranked_zone_file)]
    index_list[[length(index_list) + 1L]] <- file_check
    
    if (!all(file_check$exists)) {
      miss <- file_check[exists == FALSE, scenario]
      cat0("  [SKIP METHOD] incomplete ranked maps for ", m)
      cat0("    Missing scenarios: ", paste(miss, collapse = ", "))
      next
    }
    
    for (rank_k in ranks_to_plot) {
      
      files <- list(
        rast(reference_file)[[1]],
        get_ranked_zone_layer(m, ssp245_order[1], rank_k),
        get_ranked_zone_layer(m, ssp245_order[2], rank_k),
        get_ranked_zone_layer(m, ssp245_order[3], rank_k),
        get_ranked_zone_layer(m, "normal", rank_k),
        get_ranked_zone_layer(m, ssp585_order[1], rank_k),
        get_ranked_zone_layer(m, ssp585_order[2], rank_k),
        get_ranked_zone_layer(m, ssp585_order[3], rank_k)
      )
      
      if (any(vapply(files, is.null, logical(1)))) {
        cat0(
          "  [SKIP RANK] failed to load one or more rank-",
          rank_k,
          " layers for ",
          m
        )
      ...
      # facet_wrap(ncol = 4) fills panels by row.
      # This order reproduces Figure 5 layout:
      # row 1: Original | SSP245 2011-2040 | SSP245 2041-2070 | SSP245 2071-2100
      # row 2: Predicted reference | SSP585 2011-2040 | SSP585 2041-2070 | SSP585 2071-2100
      titles <- c(
        "Original",
        "SSP245\n2011-2040",
        "SSP245\n2041-2070",
        "SSP245\n2071-2100",
        paste0(rank_labels[as.character(rank_k)], "\npredicted reference"),
        "SSP585\n2011-2040",
        "SSP585\n2041-2070",
        "SSP585\n2071-2100"
      )
      
      fig_label <- figure_labels[as.character(rank_k)]
      plot_title <- paste0(
        "Appendix Figure ", fig_label, ": ",
        rank_labels[as.character(rank_k)], " ecosystem niche maps (",
        map_method_labels[m], ")"
      )
      ...
      index_list[[length(index_list) + 1L]] <- data.table(
        ...
        ranked_zone_file = c(
          reference_file,
          rep(file.path(ranking_root, m, ssp245_order, "ranked_zone.tif"), 1),
          file.path(ranking_root, m, "normal", "ranked_zone.tif"),
          file.path(ranking_root, m, ssp585_order, "ranked_zone.tif")
        ),
        exists = TRUE
      )
    }
  ...
  if (length(index_list) > 0) {
    index_dt <- rbindlist(index_list, fill = TRUE)
    out_idx <- file.path(tab_dir, "Appendix_ranked_ecosystem_niche_map_index.csv")
    fwrite(index_dt, out_idx)
    cat0("[SAVED] ", out_idx)
  }
})
