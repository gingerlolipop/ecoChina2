...
predict_cores <- max(1L, parallel::detectCores() - 2L)

# Run the first one first if you want to assign/check maps before running the rest.
...
remove_tmp_files <- function(filepath) {
  ...
  files <- c(filepath, paste0(base, ".aux.xml"), paste0(base, ".ovr"), paste0(base, ".tfw"))
...
calculate_dual_suitability <- function(climate, soil) {
  # Missing soil means unknown soil suitability, not zero suitability.
  climate * soil
}

predict_one_model <- function(model_file, object_name, raster_dir, out_file, template, label) {
  ...
  p <- mcPredict_terra(s, m, tmp_file)
  p <- resample(p, template, method = "bilinear")
  names(p) <- label
  writeRaster(p, out_file, overwrite = TRUE,
              wopt = list(datatype = "FLT4S", gdal = c("COMPRESS=LZW")))
...
for (mrow in seq_len(nrow(model_set))) {
  ...
  for (i in zones) {
    ...
    pclim <- predict_one_model(
      model_file  = file.path(clm_mod_dir, paste0(model_set$clm_prefix[mrow], i, ".Rdata")),
    ...
    psoil <- predict_one_model(
      model_file  = file.path(soil_mod_dir, paste0(model_set$soil_prefix[mrow], i, ".Rdata")),
    ...
    if (is.null(psoil)) { rm(pclim); gc(); next }
    ...
    writeRaster(dual, dual_file, overwrite = TRUE,
                wopt = list(datatype = "FLT4S", gdal = c("COMPRESS=LZW")))
    
    cat("[DONE]", method, "normal | zone", i, "\n")
    rm(pclim, psoil, dual); gc()
  }
}
...
for (mrow in seq_len(nrow(model_set))) {
  ...
  for (scenario in scenarios) {
    for (period in time_periods) {
      ...
      out_dir_clm_fut <- file.path(base_dir, "clim suitability", method, scen_name)
      out_dir_dual_fut <- file.path(base_dir, "dual suit", method, scen_name)
